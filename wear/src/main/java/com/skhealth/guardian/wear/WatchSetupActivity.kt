...
class WatchSetupActivity : Activity() {
    ...
    private fun render() {
        ...
        root.addView(label("SK Guardian", if (compact) 17f else 19f, text, true).apply { gravity = Gravity.CENTER })
        ...
        stateText = label("", if (compact) 17f else 19f, amber, true).apply { gravity = Gravity.CENTER }
        ...
        val spo2Card = metricCard("SpO₂", blue).also { spo2Value = it.second }
        val hrCard = metricCard("Nabız", red, "bpm").also { heartRateValue = it.second }
        ...
        root.addView(label("Ayarlar telefondan yönetilir", 10.5f, muted).apply {
    ...
    private fun metricCard(title: String, accent: Int, unit: String = "%"): Pair<LinearLayout, TextView> {
        ...
        val box = card(surface2).apply {
            gravity = Gravity.CENTER
            addView(label(title, 12f, muted).apply { gravity = Gravity.CENTER })
            addView(value)
    ...
    private fun refresh() {
        ...
        if (!permissionsOk) {
            stateText.text = "○ Kurulum gerekli"; stateText.setTextColor(amber)
