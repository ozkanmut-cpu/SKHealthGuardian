...
class WearReadingService : WearableListenerService() {
    ...
    override fun onMessageReceived(event: MessageEvent) {
        ...
        if (p.size < 5) return
        val reading = HealthReading(timestampMs=p[0].toLongOrNull()?:System.currentTimeMillis(), spo2=p[1].toIntOrNull(), heartRate=p[2].toIntOrNull(), valid=p[3].toBooleanStrictOrNull()?:false, source=p[4])
        ...
        e.evaluate(reading).forEach { AlertDispatcher(this).dispatch(it, recent) }
    }
}
