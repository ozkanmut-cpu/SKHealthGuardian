...
class ExportActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        exportCard.addView(UiStyle.iconLabel(this, SkIcon.DOWNLOAD, "Dışa aktar", UiStyle.BLUE, UiStyle.TEXT, 24, 19f, true))
        exportCard.addView(UiStyle.text(this, "CSV dosyaları analiz için, JSON dosyası ise tam uygulama yedeği için kullanılır.", 14f, UiStyle.MUTED).apply { setPadding(0, UiStyle.dp(this@ExportActivity, 8), 0, 0) })
        exportCard.addView(UiStyle.iconButton(this, SkIcon.CHARTS, "Ölçüm geçmişi • CSV", false).apply { setOnClickListener { createDocument("text/csv", "SKHealthGuardian_measurements.csv", "csv") } })
        exportCard.addView(UiStyle.iconButton(this, SkIcon.SYNC, "Watch ↔ PC-60FW doğrulama • CSV", false).apply { setOnClickListener { createDocument("text/csv", "SKHealthGuardian_watch_pc60_reliability.csv", "reliability_csv") } })
        exportCard.addView(UiStyle.iconButton(this, SkIcon.DOWNLOAD, "Tam uygulama yedeği • JSON").apply { setOnClickListener { createDocument("application/json", "SKHealthGuardian_backup.json", "json") } })
        root.addView(exportCard)

        val privacyCard = UiStyle.card(this)
        privacyCard.addView(UiStyle.iconLabel(this, SkIcon.STATUS_WARNING, "Gizlilik uyarısı", UiStyle.AMBER, UiStyle.AMBER, 22, 17f, true))
        privacyCard.addView(UiStyle.text(this, "JSON yedeği acil durum kişilerinin isim ve telefon numaralarını açık metin olarak içerir. Dosyayı güvenli bir yerde sakla.", 14f, UiStyle.MUTED).apply { setPadding(0, UiStyle.dp(this@ExportActivity, 8), 0, 0) })
        root.addView(privacyCard, UiStyle.sectionParams(this))

        val restoreCard = UiStyle.card(this)
        restoreCard.addView(UiStyle.iconLabel(this, SkIcon.UPLOAD, "Geri yükle", UiStyle.GREEN, UiStyle.TEXT, 24, 19f, true))
        restoreCard.addView(UiStyle.text(this, "Geri yükleme yedekte bulunan ayarları, kişileri ve geçmiş verileri mevcut verilerin yerine geçirir. Dosya önce doğrulanır ve işlemden önce tekrar onay istenir.", 14f, UiStyle.MUTED).apply { setPadding(0, UiStyle.dp(this@ExportActivity, 8), 0, 0) })
        restoreCard.addView(UiStyle.iconButton(this, SkIcon.UPLOAD, "JSON yedeğinden geri yükle", false).apply { setOnClickListener { chooseBackup() } })
        restoreCard.addView(UiStyle.iconLabel(this, SkIcon.INFO, "Desteklenen şemalar: v3, v2 ve v1", UiStyle.MUTED, UiStyle.MUTED, 18, 13f, false).apply { setPadding(0, UiStyle.dp(this@ExportActivity, 10), 0, 0) })
