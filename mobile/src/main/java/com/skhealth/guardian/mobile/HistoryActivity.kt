...
class HistoryActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(UiStyle.title(this, "Ölçüm geçmişi"))
        root.addView(UiStyle.subtitle(this, if (rows.isEmpty()) "Henüz ölçüm kaydı yok" else "Son ${rows.size} kayıt • en yeni ölçüm üstte"))
        ...
        if (rows.isEmpty()) {
            root.addView(UiStyle.card(this).apply {
                addView(UiStyle.text(this@HistoryActivity, "Ölçümler burada görünecek", 18f, UiStyle.TEXT, true))
                addView(UiStyle.text(this@HistoryActivity, "Galaxy Watch veya PC-60FW'den geçerli veri geldiğinde geçmiş otomatik oluşur.", 14f, UiStyle.MUTED).apply { setPadding(0, UiStyle.dp(this@HistoryActivity, 7), 0, 0) })
        ...
        rows.forEach { r ->
            ...
            header.addView(UiStyle.text(this, source, 15f, accent, true), LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f))
    ...
    private fun metric(label: String, value: String, color: Int, unit: String = "") = UiStyle.card(this).apply {
        gravity = Gravity.CENTER
        addView(UiStyle.text(this@HistoryActivity, label, 13f, UiStyle.MUTED, false, Gravity.CENTER))
