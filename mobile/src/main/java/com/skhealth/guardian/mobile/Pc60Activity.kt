...
class Pc60Activity : Activity() {
    ...
    private fun render() {
        ...
        root.addView(action("Bağlan / izlemeyi başlat","Bluetooth bağlantısını başlat",UiStyle.GREEN){if(!blePermissionsReady())requestBlePermissions() else {Pc60StatusStore.setMonitoringEnabled(this,true);ContextCompat.startForegroundService(this,Intent(this,Pc60BleService::class.java))}},UiStyle.sectionParams(this));root.addView(action("Cihazı yeniden tara","PC-60FW için yeni tarama başlat",UiStyle.BLUE){if(!blePermissionsReady())requestBlePermissions() else {Pc60StatusStore.setMonitoringEnabled(this,true);ContextCompat.startForegroundService(this,Intent(this,Pc60BleService::class.java).setAction(Pc60BleService.ACTION_RESCAN))}},UiStyle.sectionParams(this));root.addView(action("Durumu yenile","Ekrandaki canlı bilgileri güncelle",UiStyle.TEXT){render()},UiStyle.sectionParams(this));root.addView(action("İzlemeyi durdur","PC-60FW servisini kapat",UiStyle.RED){Pc60StatusStore.setMonitoringEnabled(this,false);stopService(Intent(this,Pc60BleService::class.java));render()},UiStyle.sectionParams(this))
    }
