...
class SpO2ReliabilityActivity : Activity() {
    ...
    private fun render() {
        ...
        if(matches.isEmpty()) root.addView(UiStyle.card(this).apply{addView(UiStyle.text(this@SpO2ReliabilityActivity,"Henüz eşzamanlı Watch-PC60 ölçümü yok.",15f,UiStyle.MUTED))},UiStyle.sectionParams(this)) else {val fmt=SimpleDateFormat("dd.MM • HH:mm:ss",Locale.forLanguageTag("tr-TR"));matches.takeLast(20).asReversed().forEach{m->val sign=if(m.diff>0) "+" else "";val hr=m.hrDiff?.let{d->"Nabız  ${m.watchHr} / ${m.pc60Hr} bpm  •  fark ${if(d>0) "+" else ""}$d"}?:"Nabız eşleşmesi yok";root.addView(UiStyle.card(this).apply{addView(UiStyle.text(this@SpO2ReliabilityActivity,fmt.format(Date(m.timestampMs)),13f,UiStyle.MUTED));addView(UiStyle.text(this@SpO2ReliabilityActivity,"SpO₂  Watch ${m.watch}%  /  PC60 ${m.pc60}%",16f,UiStyle.TEXT,true).apply{setPadding(0,5,0,0)});addView(UiStyle.text(this@SpO2ReliabilityActivity,"Fark $sign${m.diff} puan  •  $hr",14f,if(abs(m.diff)<=2)UiStyle.GREEN else UiStyle.AMBER).apply{setPadding(0,5,0,0)})},UiStyle.sectionParams(this))}}
    }
