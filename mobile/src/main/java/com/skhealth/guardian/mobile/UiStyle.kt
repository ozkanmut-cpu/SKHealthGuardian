...
object UiStyle {
    ...
    fun applyBottomInset(view: View, extraDp: Int = 0) {
        val initialLeft=view.paddingLeft; val initialTop=view.paddingTop; val initialRight=view.paddingRight; val initialBottom=view.paddingBottom
        ViewCompat.setOnApplyWindowInsetsListener(view) { v, insets ->
            val nav=insets.getInsets(WindowInsetsCompat.Type.navigationBars())
            v.setPadding(initialLeft,initialTop,initialRight,initialBottom+nav.bottom+dp(v.context,extraDp))
            insets
        }
