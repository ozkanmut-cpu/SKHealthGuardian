...
object UiStyle {
    ...
    const val PURPLE = 0xFF9B7BFF.toInt()
    ...
    fun icon(context: Context, kind: SkIcon, sizeDp: Int = 24, color: Int = TEXT, description: String? = null) =
        SkIconView(context, kind, color).apply {
            layoutParams = LinearLayout.LayoutParams(dp(context, sizeDp), dp(context, sizeDp))
            contentDescription = description
            importantForAccessibility = if (description.isNullOrBlank()) View.IMPORTANT_FOR_ACCESSIBILITY_NO else View.IMPORTANT_FOR_ACCESSIBILITY_YES
        }
    ...
    fun iconLabel(
        ...
        addView(icon(context, kind, iconSizeDp, iconColor, label))
    ...
    fun sectionParams(context: Context, top: Int = 12) =
    ...
    fun subtitle(context: Context, value: String) = text(context, value, 15f, MUTED).apply {
        setPadding(0, 0, 0, dp(context, 16))
    }

    fun sectionTitle(context: Context, value: String) = text(context, value, 20f, TEXT, true).apply {
    ...
    fun detailHeader(activity: Activity, title: String, subtitle: String? = null) = LinearLayout(activity).apply {
        ...
        val back = icon(activity, SkIcon.CHEVRON_LEFT, 26, TEXT, "Geri").apply {
        ...
        if (!subtitle.isNullOrBlank()) {
            addView(text(activity, subtitle, 14f, MUTED).apply { setPadding(dp(activity, 48), 0, 0, dp(activity, 12)) })
        }
    }
    ...
    fun appBottomNavigation(activity: Activity, active: String): LinearLayout = LinearLayout(activity).apply {
        ...
        fun item(kind: SkIcon, label: String, key: String, target: Class<out Activity>) = LinearLayout(activity).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
            ...
            val icon = icon(activity, kind, 24, color, label)
            addView(icon, LinearLayout.LayoutParams(dp(activity, 28), dp(activity, 28)))
            addView(text(activity, label, 11f, color, selected, Gravity.CENTER).apply { setPadding(0, dp(activity, 4), 0, 0) })
            setOnClickListener {
                if (!selected) activity.startActivity(Intent(activity, target).addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT))
            }
            layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f)
        }

        addView(item(SkIcon.HOME, "Ana ekran", "home", MainActivity::class.java))
        addView(item(SkIcon.CHARTS, "Grafikler", "charts", HistoryChartActivity::class.java))
        addView(item(SkIcon.EVENTS, "Olaylar", "events", AlarmTimelineActivity::class.java))
        addView(item(SkIcon.SETTINGS, "Ayarlar", "settings", SettingsHubActivity::class.java))
    }
    ...
    fun field(context: Context, label: String, value: String = "", numeric: Boolean = false) = EditText(context).apply {
        ...
        background = rounded(SURFACE_2, 14, 0xFF303A46.toInt(), 1, context)
    ...
    fun check(context: Context, label: String, checked: Boolean = false) = CheckBox(context).apply {
        text = label
        isChecked = checked
        setTextColor(TEXT)
        textSize = 16f
        buttonTintList = android.content.res.ColorStateList.valueOf(BLUE)
        setPadding(dp(context, 4), dp(context, 8), 0, dp(context, 8))
    }
    ...
    fun applyBottomInset(view: View, extraDp: Int = 0) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(view) { v, insets ->
            ...
            v.setPadding(l, t, r, b + nav.bottom + dp(v.context, extraDp))
            insets
        }
