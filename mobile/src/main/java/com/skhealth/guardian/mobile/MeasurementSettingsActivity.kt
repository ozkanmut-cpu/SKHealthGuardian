...
class MeasurementSettingsActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply { text = "Ölçüm ve alarm ayarları"; textSize = 24f })
        root.addView(TextView(this).apply { text = "Galaxy Watch ve PC-60FW için tüm değiştirilebilir ölçüm/alarm değerleri"; textSize = 14f })

        section("Genel alarm eşikleri")
        val spo2Critical = number("Saat kritik SpO₂ (%)", cfg.spo2CriticalImmediate)
        val spo2Low = number("Saat düşük SpO₂ (%)", cfg.spo2LowThreshold)
        val spo2Confirm = number("Saat düşük SpO₂ doğrulama sayısı", cfg.spo2ConfirmCount)
        ...
        listOf(hrLow, hrLowConfirm, stale, escalation).forEach(root::addView)

        section("Galaxy Watch ölçüm zamanlaması")
        ...
        listOf(watchInterval, watchConfirmDelay, retry1, retry2).forEach(root::addView)

        section("PC-60FW parmak oksimetre")
        val pcAlarm = number("Alarm değerlendirme eşiği (≤ %)", AppSettings.pc60AlarmThreshold(this))
        val pcConfirm = number("Düşük SpO₂ bekleme süresi (dk)", AppSettings.pc60ConfirmMinutes(this))
        val pcRecovery = number("Toparlanmış sayılacağı eşik (> %)", AppSettings.pc60RecoveryThreshold(this))
    ...
    private fun section(title: String) {
        root.addView(TextView(this).apply { text = title; textSize = 18f; setPadding(0, 28, 0, 8) })
    }
